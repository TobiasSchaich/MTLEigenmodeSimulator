function [R,C,L,G] = CalculateRCLG_coated(f, pos_vec, a, b,  ed_diel,  sigma_cond, LossTanDiel)
...
%Conductance
G=zeros(N_cond); % Not yet implemented
...
P_self=eye(N_cond).*(-1/(2*pi*eps0*ed_diel*(1-1i*LossTanDiel)*gd*a).*Z0(b)./Z1(a)-1/(2*pi*eps0*ga*a)*...
    Z1(b)/Z1(a)*(besselh(0,2*ga*pos_vec(2,:))-besselh(0,ga*b))/besselh(1,ga*b));
...
for ind1=1:N_cond
    for ind2=(ind1+1):N_cond
        P_mutual(ind1,ind2)=-1/(2*pi*a*ga*eps0)*Z1(b)/Z1(a).*...
            (besselh(0,ga*norm(pos_image_vec(:,ind1)-pos_vec(:,ind2)))-besselh(0,ga*norm(pos_vec(:,ind1)-pos_vec(:,ind2))))/besselh(1,ga*b);
